...
import io
import numpy as np
from ml import UNetInferencer
from PIL import ImageOps

# data queue
import queue
data_queue_raw = queue.Queue(maxsize=20)

# Global flag for ML processing
...
class MainWidget(QtWidgets.QMainWindow):
    def __init__(self, cast, parent=None):
        ...
        ip = QtWidgets.QLineEdit("192.168.1.108")
...
def newProcessedImage(image, width, height, sz, micronsPerPixel, timestamp, angle, imu):
    global ml_processing_enabled
    
    if ml_processing_enabled:
        # Put the data into the queue for ML processing
        try:
            data_queue_raw.put((image, width, height, sz, micronsPerPixel, timestamp), block=False)
        except queue.Full:
            # If queue is full, skip this frame to avoid blocking
            pass
    else:
        # Direct display without ML processing
        bpp = sz / (width * height)
        if bpp == 4:
...
class ProcessImage:
    def __init__(self, inferencer, data_queue, signaller):
    ...
    def run(self):
        ...
        try:
            image_bytes, width, height, sz, micronsPerPixel, timestamp = self.data_queue.get()
            # BUG: 缺少负面样本的处理，例如没有血管但是还有很多白色的灰色的区域如何判断呢

            bpp = sz / (width * height)
            qimg = self._to_qimage(image_bytes, width, height, bpp)

            print("[ProcessImage] Processing image with ML inferencer")
            input_pil = self._qimage_to_pil(qimg)

            cropped = self._crop_alpha_bbox(input_pil)
            resized = self._resize_and_pad(cropped)
            result_mask = self.inferencer.predict_PIL(resized)

            resized_qimg = QtGui.QImage(
                resized.tobytes(), resized.width, resized.height, QtGui.QImage.Format_RGBA8888
            )

            combined_qimg = self._overlay_mask(resized_qimg, result_mask)
            self._post_to_gui(combined_qimg)



        except Exception as e:
            print(f"[ProcessImage] Error: {e}")
            # fallback: post original image
            fallback_qimg = self._to_qimage(image_bytes, width, height, bpp)
            self._post_to_gui(fallback_qimg)
            

    def _to_qimage(self, image_bytes, width, height, bpp):
        if bpp == 4:
            return QtGui.QImage(image_bytes, width, height, QtGui.QImage.Format_ARGB32)
        else:
            return QtGui.QImage(image_bytes, width, height, QtGui.QImage.Format_Grayscale8)

    def _qimage_to_pil(self, qimg):
        pil = Image.fromqimage(qimg)
        if pil.mode != "RGBA":
            pil = pil.convert("RGBA")
        return pil

    def _crop_alpha_bbox(self, pil_img):
    ...
    def _resize_and_pad(self, image):
        ...
        elif w < TARGET_WIDTH and h < TARGET_HEIGHT:
            scale = min(TARGET_WIDTH / w, TARGET_HEIGHT / h)
            new_size = (int(w * scale), int(h * scale))
            image = image.resize(new_size, Image.Resampling.LANCZOS)

        return self._pad_to_bottom_right(image, (TARGET_WIDTH, TARGET_HEIGHT))

    def _pad_to_bottom_right(self, image, target_size, fill=(0, 0, 0)):
        bg = Image.new(image.mode, target_size, fill)
        bg.paste(image, (0, 0))  # paste at top-left
        return bg

    def _overlay_mask(self, qimg, result_mask):
        width = qimg.width()
        height = qimg.height()
        ptr = qimg.bits()
        arr = np.frombuffer(ptr, dtype=np.uint8).reshape(height, width, 4)  # ARGB

        # Resize mask to match
        if result_mask.shape != (height, width):
        ...
        # Green overlay
        mask_active = result_mask > 127
        alpha = 0.1
        arr[mask_active, 0] = arr[mask_active, 0] * (1 - alpha)            # B
        arr[mask_active, 1] = arr[mask_active, 1] * (1 - alpha) + 255 * alpha  # G
        arr[mask_active, 2] = arr[mask_active, 2] * (1 - alpha)            # R

        return QtGui.QImage(arr.data, width, height, QtGui.QImage.Format_RGB32)


    def _post_to_gui(self, qimage):
        self.signaller.usimage = qimage.copy()
        evt = ImageEvent()
        QtCore.QCoreApplication.postEvent(self.signaller, evt)
...
def main():
    ...
    try:
        ...
        print("Running without ML inference")
        inferencer = None
        ml_processing_enabled = False
    
    # Create image processor
    if inferencer:
        image_processor = ProcessImage(inferencer, data_queue_raw, signaller)
    else:
    ...
    # Create a timer to process images from the queue
    if image_processor:
        timer = QtCore.QTimer()
        timer.timeout.connect(image_processor.run)
        timer.start(33)  # Process images every 33ms (~30 FPS)
