...
# Clarius native libraries live in data/clarius_sdk/lib (see its README).
_lib_dir = os.path.join(os.path.dirname(__file__), '..', 'clarius_sdk', 'lib')
...
if sys.platform.startswith("linux"):
    libcast_handle = ctypes.CDLL(os.path.join(_lib_dir, "libcast.so"), ctypes.RTLD_GLOBAL)._handle
    pyclariuscast = ctypes.cdll.LoadLibrary(os.path.join(_lib_dir, "pyclariuscast.so"))
