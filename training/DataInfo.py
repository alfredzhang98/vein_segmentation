...
class DataInfo:
    def __init__(self):
        ...
        # split-specific augmented directories
        self.images_aug_train_dir = self.images_aug_dir / "train"
        self.images_aug_val_dir   = self.images_aug_dir / "val"
        self.images_aug_test_dir  = self.images_aug_dir / "test"
        self.masks_aug_train_dir  = self.masks_aug_dir / "train"
        self.masks_aug_val_dir    = self.masks_aug_dir / "val"
        self.masks_aug_test_dir   = self.masks_aug_dir / "test"
        self.meta_file      = self.base_dir / f"meta_{self.test_name}_{self.test_id}.csv"
        self.target_aug_times = {
            "true": 20,
        }
        self.mask_class = {"background": 0, "vein": 255}
    ...
    def ensure_dirs(self):
        """Create required directories and meta CSV. Call explicitly before writing data."""
        for d in [self.base_dir, self.images_dir, self.masks_dir,
                  self.images_aug_dir, self.masks_aug_dir, self.checkpoints_dir,
                  self.images_aug_train_dir, self.images_aug_val_dir, self.images_aug_test_dir,
